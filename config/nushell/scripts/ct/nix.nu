...
export def nix-smoke [
    ...
    mut checks = [
        (
            (_smoke-check
                "PATH has ~/.local/bin"
                (_path-has ($env.HOME | path join ".local/bin"))
                ($env.HOME | path join ".local/bin")
            )
        )
        (
            (_smoke-check
                "PATH has /nix/var/nix/profiles/default/bin"
                (_path-has "/nix/var/nix/profiles/default/bin")
                "/nix/var/nix/profiles/default/bin"
            )
        )
        (
            (_smoke-check
                $"PATH has /etc/profiles/per-user/($env.USER)/bin"
                (_path-has $"/etc/profiles/per-user/($env.USER)/bin")
